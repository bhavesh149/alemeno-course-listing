...
const Course = ({ id, image, course_name, creator, actual_price, discounted_price, rating_count, rating_star, category, isCompleted, onCompleteCourse,isDashboard  }) => {
  ...
  return (
    <CourseCard>
      ...
      <div className='item-btns flex'>
        ...
        <button className='item-btn add-to-cart-btn' onClick={() => addToCart(id, image, course_name, creator, discounted_price, category)}>Add to cart</button>
        {/* <button className='item-btn complete-course-btn' onClick={() => onCompleteCourse(id)}>Complete Course</button> */}
        {isDashboard && (
          <button className='item-btn complete-course-btn' onClick={() => onCompleteCourse(id)}>Complete Course</button>
        )}
      </div>
      {isCompleted && <div className="completion-status">Course Completed 100%</div>}
    </CourseCard>
  );
}
